...
using MccSoft.PushNotification.ComponentTests.Infrastructure;
using MccSoft.PushNotification.Http;
...
namespace MccSoft.PushNotification.ComponentTests
{
    public class TestBase : Testing.TestBase, IDisposable
    {
        ...
        protected TestBase()
        {
            ...
            var db = ResolveFromTestScope<PushNotificationDbContext>();
        ...
        protected void WithDbContext(Action<PushNotificationDbContext> action)
        {
            using var scope = TestServer.Host.Services.CreateScope();
            var db = scope.ServiceProvider.GetRequiredService<PushNotificationDbContext>();
            action(db);
        }

        protected T WithDbContext<T>(Func<PushNotificationDbContext, T> action)
        {
            using var scope = TestServer.Host.Services.CreateScope();
            var db = scope.ServiceProvider.GetRequiredService<PushNotificationDbContext>();
            return action(db);
