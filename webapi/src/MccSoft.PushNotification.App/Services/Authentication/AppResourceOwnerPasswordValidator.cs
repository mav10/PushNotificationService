...
namespace MccSoft.PushNotification.App.Services.Authentication
{
        ...
        where TUser : class
    {
        ...
        public AppResourceOwnerPasswordValidator(
        ...
        public virtual async Task ValidateAsync(ResourceOwnerPasswordValidationContext context)
        {
            ...
            if (user != null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    _logger.LogInformation(
                        "Credentials validated for username: {username}",
                        context.UserName
                    );
                ...
                else if (result.IsLockedOut)
                {
                    ...
                    await _events.RaiseAsync(
                        new UserLoginFailureEvent(context.UserName, "locked out", false, clientId)
                    );
                    context.Result = new GrantValidationResult(
                ...
                else if (result.IsNotAllowed)
                {
                    ...
                    await _events.RaiseAsync(
                        new UserLoginFailureEvent(context.UserName, "not allowed", false, clientId)
                    );
                }
                else
                {
            ...
            else
            {
                ...
                await _events.RaiseAsync(
                    new UserLoginFailureEvent(context.UserName, "invalid username", false, clientId)
                );
            }
